...
class ModalFusionLayer_TWO(nn.Module):
    def __init__(self, in_dim, out_dim, multi, img_dim, txt_dim):
        super(ModalFusionLayer_TWO, self).__init__()
    ...
    def forward(self, modal1_emb, modal2_emb, modal3_emb):
        ...
        for i in range(self.multi):
            ...
            x_modal3 = self.modal3_layers[i](modal3_emb)
            # x_stack = torch.stack((x_modal1, x_modal2, x_modal3), dim=1)
            x_stack = torch.stack((modal2_emb, modal3_emb), dim=1)
            attention_scores = self.ent_attn(x_stack).squeeze(-1)
            attention_weights = torch.softmax(attention_scores, dim=-1)
            context_vectors = torch.sum(attention_weights.unsqueeze(-1) * x_stack, dim=1)
            x_mm.append(context_vectors)
...
class RelMoE(BaseModel):
    def __init__(self, args):
        ...
        self.fuse_e = ModalFusionLayer_TWO(
            in_dim=args.dim,
        ...
        self.fuse_r = ModalFusionLayer_TWO(
            in_dim=args.r_dim,
            out_dim=self.fuse_out_dim,
            multi=2,
            img_dim=args.r_dim,
    ...
    def forward(self, batch_inputs):
        ...
        e_feat = self.bn(self.conv(e_embed.unsqueeze(0).permute(0, 2, 1).unsqueeze(-1))).squeeze(-1).squeeze(0).permute(1, 0)
        e_img_feat = self.bn(self.conv(e_img_embed.unsqueeze(0).permute(0, 2, 1).unsqueeze(-1))).squeeze(-1).squeeze(0).permute(1, 0)
        e_txt_feat = self.bn(self.conv(e_txt_embed.unsqueeze(0).permute(0, 2, 1).unsqueeze(-1))).squeeze(-1).squeeze(0).permute(1, 0)

        if not self.training:
            return [pred_s, pred_i, pred_d, pred_mm], [atten_s, atten_i, atten_t, attn_f], [e_feat, e_img_feat, e_txt_feat]
        else:
            return [pred_s, pred_i, pred_d, pred_mm], [disen_str, disen_img, disen_txt], [e_feat, e_img_feat, e_txt_feat]
    ...
    def loss_func(self, output, target, e_embed):
        ...
        loss_struct_img = self.cliploss(e_embed[0], e_embed[1])
        loss_struct_txt = self.cliploss(e_embed[0], e_embed[2])
        loss_img_txt = self.cliploss(e_embed[1], e_embed[2])

        # return loss_s + loss_i + loss_d + loss_mm
        # return loss_s + loss_i + loss_d + loss_mm + (loss_struct_img + loss_struct_txt + loss_img_txt)*0.01
        return loss_s + loss_i + loss_d + (loss_struct_img + loss_struct_txt + loss_img_txt)*0.01
...
#     def forward(self, P_embeds, Q_embeds, labels):
#         P_embeds = P_embeds / P_embeds.norm(dim=-1, keepdim=True)
#         Q_embeds = Q_embeds / Q_embeds.norm(dim=-1, keepdim=True)
#         similarity = torch.matmul(P_embeds, Q_embeds.T) 

#         target_similarity = labels.float()
#         print('target_similarity shape is ===', target_similarity.shape, 'similarity shape is ===', similarity.shape)

#         positive_loss = target_similarity * (1.0 - similarity)
#         negative_loss = (1.0 - target_similarity) * F.relu(self.margin - similarity)
...
#         sim_i2t = P_feat @ Q_feat.t() / self.temp
#         sim_t2i = Q_feat @ P_feat.t() / self.temp

#         batch_size = P_feat.size(0)
#         sim_targets = torch.zeros(batch_size, batch_size, device=P_feat.device)
#         for i in range(batch_size):
#             sim_targets[i, i] = 1.0  # Set positive samples for each image-text pair

#         sim_targets = sim_targets / sim_targets.sum(1, keepdim=True)  # Normalize to get probabilities
...
class ClipLoss(nn.Module):
    ...
    def forward(self, P_feat, Q_feat):
        ...
        logits_per_P = P_feat @ Q_feat.T / self.temp
        logits_per_Q = Q_feat @ P_feat.T / self.temp

        # 生成标签
        labels = torch.arange(logits_per_P.shape[0], device=P_feat.device, dtype=torch.long)

        # 计算损失
        total_loss = (
            F.cross_entropy(logits_per_P, labels) +
            F.cross_entropy(logits_per_Q, labels)
        ) / 2
